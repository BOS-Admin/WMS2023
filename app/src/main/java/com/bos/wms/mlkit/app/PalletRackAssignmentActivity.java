...
public class PalletRackAssignmentActivity extends AppHelperActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("Pallet Rack Assignment");
    }
    ...
    public void ProcessInputComplete(){
        ProcessAssignment(CurrentScannedPallet);

        scannedPallet.setText("Waiting For Pallet");
        CurrentScannedPallet = null;
    }

    public void ProcessAssignment(String pallet){

        if(pallet != null){
            mainProgressDialog = ProgressDialog.show(this, "", "Assigning Bin Please Wait...", true);

            try {

                Logger.Debug("API", "ProcessAssignment - Process Assigning For Bin '" + pallet + "'");

                BasicApi api = APIClient.getNewInstanceStatic(IPAddress,60).create(BasicApi.class);
                CompositeDisposable compositeDisposable = new CompositeDisposable();

                compositeDisposable.addAll(
                        api.GetBinRackAssignment(pallet, General.getGeneral(this).UserID)
                                .subscribeOn(Schedulers.io())
                                .observeOn(AndroidSchedulers.mainThread())
                                .subscribe((s) -> {
                                    if(s != null){
                                        String result = s.string();
                                        Logger.Debug("API", "ProcessAssignment - Returned: " + result);
                                    ...
                                    if(throwable instanceof HttpException){
                                        ...
                                        Logger.Debug("API", "ProcessAssignment - Error In HTTP Response: " + error);
                                    }else {
                                        Logger.Error("API", "ProcessAssignment - Error In API Response: " + throwable.getMessage());
                                    }
                ...
                Logger.Error("API", "ProcessAssignment - Error Connecting: " + ex.getMessage());
                mainProgressDialog.cancel();
            }
