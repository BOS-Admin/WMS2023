...
class PutAwayActivity : AppCompatActivity() {
    ...
    private fun proceed() {
        ...
        try {
            ...
            {
                showScanMessage("Rack or Bins is empty",Color.RED)
                updatingText=false
                updatingText1=false
                textBoxScanned.requestFocus()
                textLocationScanned.requestFocus()
            ...
            var modelItems:ArrayList<PutAwayItemModel>  = arrayListOf()
            for(it in boxes)
                modelItems.add(PutAwayItemModel(it.value))

            var model=
                PutAwayModel (toLocation,general.UserID,modelItems)

            //Log.i("Ah-log",itemsStr);

            api= APIClient.getInstance(IPAddress,false).create(BasicApi::class.java)
            compositeDisposable.addAll(
                api.RackPutAway(model)
                    .subscribeOn(Schedulers.io())
                    // .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(
                        {s->
...
textLocationScanned.setText(general.boxNb )
        toLocation=general.boxNb
            textLocationScanned.isEnabled=false
        textBoxScanned.requestFocus()
...
//                    Beep()
//                    showScanMessage("Invalid rack :$location",Color.RED)
...
//                    Beep()
//                    showScanMessage("Finish before changing the Rack !!! Current Rack ($toLocation), Rack Scanned ($location)",Color.RED)
//                    textLocationScanned.setText("")
//                    updatingText1=false;
//                    return;
//                }
