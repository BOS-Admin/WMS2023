...
class ScanRackStockTakeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       // txtScanDestination.setShowSoftInputOnFocus(false);
        ...
        lblDescription.text="Please scan your user code and the rack code"
        lblBox.text = "Rack Code"
    ...
    fun ValidateBin(binBarcode:String,packingTypeId:Int,userId:Int,locationId:Int,destination:String) {
        ...
        try {
            ...
            api= APIClient.getInstance(general.ipAddress ,true).create(
                BasicApi::class.java)
            compositeDisposable.addAll(
                api.ValidateRack (general.transactionType,binBarcode)
                    .subscribeOn(Schedulers.io())
                    // .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(
                        {s->
                            if(s!==null){
                                Log.i("AH-Log-Pack","id "+s.id)
                                general.boxNb=binBarcode
                                general.stockType=102
                                general.saveGeneral(applicationContext)
                                if(general.transactionType==100){
                                    lblError1.text = "Success"
                                    lblError1.setTextColor(Color.GREEN)
                                    val intent = Intent (applicationContext, StockTakeCountActivity::class.java)
                                    startActivity(intent)
                                    finish()
                                }
                                else{
                                    lblError1.text = "Success"
                                    lblError1.setTextColor(Color.GREEN)
                                    val intent = Intent (applicationContext, StockTakeRackDCActivity::class.java)
                                    startActivity(intent)
                                    finish()
                                }
