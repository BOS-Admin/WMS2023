...
class ScanForCountStockTakeActivity : AppCompatActivity() {
    ...
    fun ValidateBin(binBarcode:String,packingTypeId:Int,userId:Int,locationId:Int,destination:String) {
        ...
        try {
            ...
            compositeDisposable.addAll(
                api.ValidateBinForStockTake(binBarcode,100,userId,locationId)
                    .subscribeOn(Schedulers.io())
                    // .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(
                        {s->
                            if(s!==null ){
                                Log.i("AH-Log-ScanForCountStockTake","type "+s.binTypeId)
