 ...
 * @version: V1.12
 */
public class FillBinActivity extends BasActivity implements
...
try {
    ...
    private void initHoldBluetooth() {
        ...
        mHoldBluetooth.initHoldBluetooth(FillBinActivity.this,updateList);
    }
    ...
    private Boolean addFilterList(DeviceModule deviceModule, boolean isRefresh){
        ...
        if (isRefresh)
            mainRecyclerAdapter.notifyDataSetChanged();*/
            mHoldBluetooth.setDevelopmentMode(FillBinActivity.this);//设置是否进入开发模式
...
private void ReadRfid(int[] Indexes, int ReadTimeInms){
    ...
    private void initDataListener() {
        ...
        HoldBluetooth.OnReadDataListener dataListener = new HoldBluetooth.OnReadDataListener() {
            ...
            private void ReadMultiRFID(ArrayAdapter adapter){
    ...
    private void setPopWindow(View v){
        new PopWindowMain(v, FillBinActivity.this, new PopWindowMain.DismissListener() {
            @Override
    ...
    private void ProceedRFIDToAPI() {
        try
        {
            ...
            FillBinModel bin=new FillBinModel("",1, hmList.keySet().toArray(new String[hmList.size()]),WeightValue,WeightMac,RFIDMac,General.getMacAddress());
            Call<ResponseBody> call = api.ProceedFillBin(bin);
    ...
    private void initPermission(){
        PermissionUtil.requestEach(FillBinActivity.this, new PermissionUtil.OnPermissionListener() {
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (mHoldBluetooth.bluetoothState()){
                            if (Analysis.isOpenGPS(FillBinActivity.this))
                                refresh();
                            else
                                startLocation();
                        }
