...
open class MacysRfidLotBondEnhancedActivity : BasActivity(), RFIDListener {
    ...
    override fun notifyListener(device: RFIDDevice, tag_model: Tag_Model) {
        try {
            ...
            synchronized(hmListLock) {
                ...
                //Added This So That Any Tag Is Detected
                if (!rfid.lowercase().startsWith("bbb") && !rfid.lowercase().startsWith("ddd")) {
                    if (!bbbMap.containsKey(rfid)) {
                        bbbMap[rfid] = tag_model
                    }
                }else {
                    addFailStep("ERROR! Read BBB/DDD RFID", rfid)
                }
            }
    ...
    fun setPopWindow(v: View) {
        PopWindowMain(
            v, this@MacysRfidLotBondEnhancedActivity
        ) { resetEngine ->
            //弹出窗口销毁的回调
