...
open class ItemSerialLotBondEnhancedActivity : BasActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lblRfidDevice.visibility = View.INVISIBLE
    ...
    inner class ItemSerialTextWatcher : TextWatcher {
     override fun afterTextChanged(s: Editable) {
            ...
            Thread {
                initItemSerialLotBondSession(str)
            }.start()
    ...
    //endregion
    //region start lot bond session
    open fun initItemSerialLotBondSession(itemSerial: String) {
        ...
        resetItemSerialLotBonSession()
        StartItemSerialReadSession(itemSerial)

    }
    open fun resetItemSerialLotBonSession(){
        clearSteps()
    ...
    open fun StartItemSerialReadSession(itemSerial: String){
        if(!allowSession){
        ...
        ProcessItemSerialScan(itemSerial)

    }

    fun ProcessItemSerialScan(itemSerial: String) {
        ...
        lotBondAPI(itemSerial)
    }

    //endregion
    //region lotBond
    fun lotBondAPI(itemSerial: String) {
        try {
                        ...
                        stationMessages["Success Msg"]?.let {
                            ...
                            if (response == stationMessages["Success Resp"])
                                addSuccessStep(
                                    ...
                                    "Item Serial Receive Success Resp : $response",
    ...
    fun setPopWindow(v: View) {
        PopWindowMain(
            v, this@ItemSerialLotBondEnhancedActivity
        ) { resetEngine ->
            //弹出窗口销毁的回调
